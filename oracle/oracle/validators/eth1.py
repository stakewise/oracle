...
from .types import Operator, ScoringInfo, ValidatorDepositData
...
async def select_validator(
    ...
    operators = await get_operators(network, block_number)
    scoring_info = fetch_scoring(network, block_number)
    for operator in await Scoring(operators, scoring_info).sorted_operators():
        ...
        while deposit_data_index < len(deposit_datum) - 1 and not can_register:
            ...
            if can_register:
                return ValidatorDepositData(
                    ...
                    public_key=selected_deposit_data["public_key"],
                    withdrawal_credentials=selected_deposit_data[
...
async def get_operators(network: str, block_number: BlockNumber) -> List[Operator]:
...
async def fetch_scoring(
    network: str, block_number: BlockNumber
) -> Union[None, List[ScoringInfo]]:
    """Selects the next validator to register."""
    result: Dict = await execute_sw_gql_query(
        network=network,
        query=SCORING_QUERY,
